 ...
 ****************************************************************************/
// https://github.com/raspberrypi/pico-sdk/blob/ee68c78d0afae2b69c03ae1a72bf5cc267a2d94c/src/rp2350/hardware_regs/include/hardware/regs/addressmap.h#L27
#define XIP_BASE                 0x10000000
#define XIP_NOCACHE_NOALLOC_BASE 0x14000000
#define FLASH_BLOCK_ERASE_CMD    0x20
// FIXME: Boot ROM:
// Each RP2350 device contains 32 kB of mask ROM: a physically immutable memory resource
// Unlike RP2040, there is no requirement for flash binaries to have a checksummed "boot2" flash setup function at flash
//   address 0.

// FIXME: Boot RAM
// is a 1 kB (256 × 32-bit) SRAM dedicated for use by the bootrom. It is *slower than main SRAM*, as it is
//   accessed over APB, taking three cycles for a read and four cycles for a write.
// contains:
// - a copy of the flash XIP setup function (formerly known as boot2) to
// quickly reinitialise flash XIP modes following serial programming operations.
// It is physically impossible to execute code from boot RAM,
...
// within XIP_NOCACHE_NOALLOC_BASE to disable cache
#define FLASH_START_READ   (rp2040_smart_flash_start + 0x04000000) // RP2350 has different address than RP2040 !

// FIXME cleanup
#define CONFIG_RP2040_FLASH_LENGTH ( CONFIG_RP23XX_FLASH_LENGTH )
...
typedef struct rp2040_flash_dev_s
{
  ...
  uint32_t         boot_2[BOOT_2_SIZE / 4]; /* RAM copy of boot_2  : FIXME - or this is bootrom !!! */
} rp2040_flash_dev_t;
...
struct mtd_dev_s *rp23xx_flash_mtd_initialize(void)
{
  ...
  rom_functions.connect_internal_flash = ROM_LOOKUP(ROM_FUNC_CONNECT_INTERNAL_FLASH);
  rom_functions.flash_exit_xip         = ROM_LOOKUP(ROM_FUNC_FLASH_EXIT_XIP);
  rom_functions.flash_range_erase      = ROM_LOOKUP(ROM_FUNC_FLASH_RANGE_ERASE);
  rom_functions.flash_range_program    = ROM_LOOKUP(ROM_FUNC_FLASH_RANGE_PROGRAM);
  rom_functions.flash_flush_cache      = ROM_LOOKUP(ROM_FUNC_FLASH_FLUSH_CACHE);
  // FIXME: for now using impl from ROM, is there a better one? no boot2 @ RP2350 inst there?
  rom_functions.flash_enable_xip       = ROM_LOOKUP(ROM_FUNC_FLASH_ENTER_CMD_XIP);
  ...
  // FIXME: RP2350 would need a change here?!!!
  //memcpy(my_dev.boot_2, (void *)XIP_BASE, BOOT_2_SIZE);
  // rom_functions.flash_enable_xip = (flash_enable_xip_f)my_dev.boot_2 + 1;

  /* RP2350 Manual:
  *Boot RAM*:
   is used for myriad purposes during boot, including the initial pre-boot stack. After the bootrom enters the
   user application, boot RAM contains state for the user-facing ROM APIs, such as the resident partition table used for
   flash programming protection,
   and a copy of the flash XIP setup function (formerly known as boot2) to quickly reinitialise flash XIP modes
   following serial programming operations.

   Boot RAM is hardwired to permit Secure access only (Arm) or Machine-mode access only (RISC-V).
   *It is physically impossible to execute code from boot RAM*,
   regardless of MPU configuration, as it is on the APB peripheral bus
   segment, which is not wired to the processor instruction fetch ports.

   above the boot RAM base address of 0x400e0000 (defined as BOOTRAM_BASE in the SDK).
   ---
  ...
  */
  ...
  // FIXME: rom_functions are NULL now - this fails, w/o init ok, /flash exists (empty), and /dev/smart0 too, but not writeable
  if (true && memcmp(rp2040_smart_flash_start, "2040", 4) == 0)
    {
