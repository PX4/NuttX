...
/* CONFIG_IMX9_FLEXCAN_NETHIFS determines the number of physical
...
#if (CONFIG_IMX9_FLEXCAN_RXMB + CONFIG_IMX9_FLEXCAN_TXMB) > 20
# error Only 20 MB are allowed to be used
#endif
...
static uint8_t mb_address[] =
                              {
                               ...
                               0x90, 0x99, 0xa2, 0xab, 0xb4, 0xbd, 0xc6
                              };
...
{
  iomux_cfg_t tx_pin;           /* GPIO configuration for TX */
  iomux_cfg_t rx_pin;           /* GPIO configuration for RX */
  uint32_t clk_root;
...
static const struct flexcan_config_s imx9_flexcan1_config =
{
  .tx_pin      = GPIO_FLEXCAN1_TX,
  .rx_pin      = GPIO_FLEXCAN1_RX,
  .clk_root    = FLEXCAN1_CLK,
  .irq         = IMX9_IRQ_CAN1,
};
#endif

#ifdef CONFIG_IMX9_FLEXCAN2
static const struct flexcan_config_s imx9_flexcan2_config =
{
  .tx_pin      = GPIO_FLEXCAN2_TX,
  .rx_pin      = GPIO_FLEXCAN2_RX,
  .clk_root    = FLEXCAN2_CLK,
  .irq         = IMX9_IRQ_CAN2,
};
#endif

#ifdef CONFIG_IMX9_FLEXCAN3
static const struct flexcan_config_s imx9_flexcan3_config =
{
  .tx_pin      = GPIO_FLEXCAN3_TX,
  .rx_pin      = GPIO_FLEXCAN3_RX,
  .clk_root    = FLEXCAN3_CLK,
  .irq         = IMX9_IRQ_CAN3,
};
#endif

#ifdef CONFIG_IMX9_FLEXCAN4
static const struct flexcan_config_s imx9_flexcan4_config =
{
  .tx_pin      = GPIO_FLEXCAN4_TX,
  .rx_pin      = GPIO_FLEXCAN4_RX,
  .clk_root    = FLEXCAN4_CLK,
  .irq         = IMX9_IRQ_CAN4,
};
#endif

#ifdef CONFIG_IMX9_FLEXCAN5
static const struct flexcan_config_s imx9_flexcan5_config =
{
  .tx_pin      = GPIO_FLEXCAN5_TX,
  .rx_pin      = GPIO_FLEXCAN5_RX,
  .clk_root    = FLEXCAN5_CLK,
  .irq         = IMX9_IRQ_CAN5,
};
#endif
...
#ifdef CONFIG_IMX9_FLEXCAN3
static struct imx9_driver_s g_flexcan3;
#endif

#ifdef CONFIG_IMX9_FLEXCAN4
static struct imx9_driver_s g_flexcan4;
#endif

#ifdef CONFIG_IMX9_FLEXCAN5
static struct imx9_driver_s g_flexcan5;
#endif
...
uint32_t imx9_bitratetotimeseg(struct flexcan_timeseg *timeseg,
...
static void imx9_setenable(uint32_t base, uint32_t enable);
static void imx9_setfreeze(uint32_t base, uint32_t freeze);
static uint32_t imx9_waitmcr_change(uint32_t base,
                                       uint32_t mask,
                                       uint32_t target_state);
static struct mb_s *flexcan_get_mb(struct imx9_driver_s *priv,
                                    uint32_t mbi);
...
static int  imx9_flexcan_interrupt(int irq, void *context,
                                      void *arg);
static void imx9_flexcan_interrupt_work(void *arg);
...
static int imx9_transmit(struct imx9_driver_s *priv)
{
  ...
  regval = getreg32(priv->base + IMX9_CAN_IMASK1_OFFSET);
  regval |= mb_bit;
  putreg32(regval, priv->base + IMX9_CAN_IMASK1_OFFSET);
  ...
  while ((f = flags) != 0)
    {
      ...
      if (rf->cs.edl) /* CAN FD frame */
        ...
        struct canfd_frame *frame = (struct canfd_frame *)priv->rxdesc_fd;
          ...
          putreg32(1 << mbi,
                   priv->base + IMX9_CAN_IFLAG1_OFFSET);
...
/****************************************************************************
 * Function: imx9_flexcan_interrupt_work
 *
 * Description:
...
static void imx9_flexcan_interrupt_work(void *arg)
{
  ...
  net_lock();
  imx9_receive(priv, flags);
  net_unlock();

  /* Mask MB again */

  modifyreg32(priv->base + IMX9_CAN_IMASK1_OFFSET, 0, IFLAG1_RX);
}
...
static void imx9_setenable(uint32_t base, uint32_t enable)
{
  ...
  if (enable)
    {
      regval  = getreg32(base + IMX9_CAN_MCR_OFFSET);
      regval &= ~(CAN_MCR_MDIS);
      putreg32(regval, base + IMX9_CAN_MCR_OFFSET);
    }
  else
    {
      regval  = getreg32(base + IMX9_CAN_MCR_OFFSET);
  ...
  imx9_waitmcr_change(base, CAN_MCR_LPMACK, 1);
}
...
static uint32_t imx9_waitmcr_change(uint32_t base, uint32_t mask,
                                       uint32_t target_state)
{
...
static uint32_t imx9_waitfreezeack_change(uint32_t base,
                                             uint32_t target_state)
{
  return imx9_waitmcr_change(base, CAN_MCR_FRZACK, target_state);
}
...
static int imx9_init_eccram(struct imx9_driver_s *priv)
{
  ...
  for (i = IMX9_CAN_MB_OFFSET; i < IMX9_CAN_MB_END; i += 4)
    {
      putreg32(0, priv->base + i);
    }

  for (i = IMX9_CAN_MB2_OFFSET; i < IMX9_CAN_MB2_END; i += 4)
    {
      putreg32(0, priv->base + i);
    }
...
static int imx9_initialize(struct imx9_driver_s *priv)
{
  ...
  imx9_setfreeze(priv->base, 1);
  if (!imx9_waitfreezeack_change(priv->base, 1))
    {
  ...
  if (!priv->canfd_capable)
    {
      regval  = getreg32(priv->base + IMX9_CAN_CTRL1_OFFSET);
      ...
      putreg32(regval, priv->base + IMX9_CAN_CTRL1_OFFSET);
    }
  else
    {
      regval  = getreg32(priv->base + IMX9_CAN_CBT_OFFSET);
      ...
      putreg32(regval, priv->base + IMX9_CAN_CBT_OFFSET);

      /* Enable CAN FD feature */

      regval  = getreg32(priv->base + IMX9_CAN_MCR_OFFSET);
      regval |= CAN_MCR_FDEN;
      putreg32(regval, priv->base + IMX9_CAN_MCR_OFFSET);

      regval  = getreg32(priv->base + IMX9_CAN_FDCBT_OFFSET);
      ...
      putreg32(regval, priv->base + IMX9_CAN_FDCBT_OFFSET);

      /* Additional CAN-FD configurations */

      regval  = getreg32(priv->base + IMX9_CAN_FDCTRL_OFFSET);
      ...
      putreg32(regval, priv->base + IMX9_CAN_FDCTRL_OFFSET);
  ...
  putreg32(IFLAG1_RX, priv->base + IMX9_CAN_IMASK1_OFFSET);

  /* Exit freeze mode */

  imx9_setfreeze(priv->base, 0);
  if (!imx9_waitfreezeack_change(priv->base, 0))
    {
...
static void imx9_reset(struct imx9_driver_s *priv)
{
  ...
  if (!imx9_waitmcr_change(priv->base, CAN_MCR_SOFTRST, 0))
    {
      nerr("Reset failed");
      return;
    }
...
#  endif
  ...
  if (!imx9_bitratetotimeseg(&priv->arbi_timing, 1, 0))
    {
  ...
  if (priv->canfd_capable)
    {
      if (!imx9_bitratetotimeseg(&priv->data_timing, 1, 1))
        {
  ...
  imx9_iomux_configure(priv->config->tx_pin);
  imx9_iomux_configure(priv->config->rx_pin);

  if (irq_attach(priv->config->irq, imx9_flexcan_interrupt, priv))
    {
