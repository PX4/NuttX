    ...
    stage('Build') {
      steps {
        script {
          ...
          def docker_nuttx = "px4io/px4-dev-nuttx:2019-10-24"
    ...
    node {
      docker.image(docker_repo).inside('-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw') {
        stage("build") {
          ...
          sh('ccache -s')
          sh('size nuttx')
          archiveArtifacts(allowEmptyArchive: false, artifacts: 'nuttx, *.elf, *.bin', fingerprint: true, onlyIfSuccessful: true)
        } // stage
        cleanWs()
      }
    }
