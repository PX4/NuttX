/****************************************************************************
 * boards/arm/at32/at32f437-mini/src/at32_progmem.c
 *
...
static struct mtd_dev_s *progmem_alloc_mtdpart(uint32_t mtd_offset,
                                                   uint32_t mtd_size);
static int init_progmem_partitions(void);
#endif
...
#if defined(CONFIG_MTD_PARTITION)
static const struct progmem_partition_s g_progmem_partition_table[] =
{
  {
    .offset  = 0x300000,    /* partition start address, must be the start address of sector */
    .size    = 0x80000,     /* partition size, must be equal to one or more sector sizes */
    .devpath = "/dev/progmem1"
  },
  {
    .offset  = 0x380000,    /* partition start address, must be the start address of sector */
    .size    = 0x40000,     /* partition size, must be equal to one or more sector sizes */
    .devpath = "/dev/progmem2"
  },
};
#endif
                                               ...
                                               uint32_t mtd_size)
{
  ...
  blocks = mtd_size / up_progmem_pagesize(startblock);

  return mtd_partition(g_progmem_mtd, startblock, blocks);
}

/****************************************************************************
 * Name: init_progmem_partitions
 *
 * Description:
 *   Initialize partitions that are dedicated to store device
 *   or firmware OTA update.
 *
 * Input Parameters:
 *   None.
 *
...
static int init_progmem_partitions(void)
{
  ...
  for (i = 0; i < nitems(g_progmem_partition_table); ++i)
    {
      const struct progmem_partition_s *part = &g_progmem_partition_table[i];
...
#ifdef CONFIG_FS_FAT
      /* Used as a mtd block device */

      ret = ftl_initialize(i, mtd);
#else
      /* Use as a char device */
...
int at32_progmem_init(void)
{
  ...
  ret = init_progmem_partitions();
  if (ret < 0)
    {
      ferr("ERROR: Failed to create progmem partition from MTD\n");
      ret = -EIO;
    }
#endif
