/****************************************************************************
 * boards/arm/at32/at32f437-mini/src/at32_adc.c
 *
...
#if defined(CONFIG_ADC) && (defined(CONFIG_AT32_ADC1) || defined(CONFIG_AT32_ADC3))
  ...
  if (!initialized)
    {
      ...
      for (i = 0; i < DEV1_NCHANNELS; i++)
        {
          at32_configgpio(g_pinlist1[i]);
        }

      /* Call at32_adcinitialize() to get an instance of the ADC interface */

      adc = at32_adcinitialize(DEV1_PORT, g_chanlist1, DEV1_NCHANNELS);
      if (adc == NULL)
        {
      ...
      for (i = 0; i < DEV2_NCHANNELS; i++)
        {
          at32_configgpio(g_pinlist2[i]);
        }

      /* Call at32_adcinitialize() to get an instance of the ADC interface */

      adc = at32_adcinitialize(DEV2_PORT, g_chanlist2, DEV2_NCHANNELS);
      if (adc == NULL)
        {
...
#endif /* CONFIG_ADC && (CONFIG_AT32_ADC1 || CONFIG_AT32_ADC3) */
